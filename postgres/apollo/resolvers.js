...
const isAuthenticated = (parent, args, context) =>
  context.authenticated == "success" ? skip : new Error("Not authenticated");

module.exports = {
  Query: {
    simple: combineResolvers(isAuthenticated, async (parent, args, context) => {
  ...
  Mutation: {
    async createdress(
      parent,
      { dressinput: { name, type, size, location, currdate } }
    ) {
      const createdress = await Dress.create({
        name,
        type,
        size,
        location,
    ...
    async updatedress(
      parent,
      { id, dressinput: { name, type, size, location, currdate } }
    ) {
      const updatedress = await Dress.update(
        { name, type, size, location, currdate },
        {
          where: {
            id,
          },
